...
const endpoints = [
    ...
    { method: 'POST', path: '/iot/v1/metrics', description: 'Submit IoT device metrics' },
    { method: 'POST', path: '/entrance/v1/movement', description: 'Record entrance movement data' }
];
...
export function rootHandler(request, reply) {
    ...
    <html>
    <head>
        <title>Telemetry API Service</title>
        <style>
    ...
    <body>
        <div class="header" style="display: flex; flex-direction: row; align-items: center;">
            ...
            <h1>Telemetry API microservice</h1>
        </div>
        <p>This service handles metrics collection and storage for Gratheon.com, including IoT sensor data from beehives.</p>
